...
Promise.resolve(r.Al(s)).then(e=>[e,1])});h=5===a.Yl&&h instanceof Array?h[0]:h,e=h instanceof Array?h.join(" "):h
;case"cp":case"copy":case"clip":let l=r.Al(e);return"string"==typeof l?[e,1]:l.then(e=>[e,1])}switch(g){case"urls":
...
;let i=n.je({u:b[2],p:d,t:null,f:1,a:"."!==b[1]?b[1]:""});return i&&i.u||[i?i.e:"No upper path",3];case"parse":
case"decode":g=e.split(" ",1)[0],g.search(/\/|%2f/i)<0?e=e.slice(g.length+1).trimLeft():g="~",b=[e=l.zl(e)],
e=a.Xl(e,null,0,p),4!==a.Yl&&(y=n.je({u:e}))?""===y.u?b=[g]:(b=y.u.split(" "),b.unshift(g)):b=b[0].split(l.D);break
...
b=l.zl(e,true).split(l.D);break;case"paste":
if(s>0)return h=r.Jl(e),h instanceof Promise?h.then(e=>[e?e.trim().replace(l.D," "):"",5]):[h?h.trim().replace(l.D," "):"",5]
;default:return null}if(c)return[b,2];if(p&&p>12)return null;let x=b[0]&&r.Il.map.has(b[0])?b.shift():null
...
l="function"==typeof l?"":""+l}catch(e){}t.clean(),t.errormsg&&(t.errormsg="")}return[l,0,e]},f=e=>{let s=r.ye
;!parseInt(e,10)||(s=parseInt(e,10),e=e.slice(e.search(/[/ ]/)+1));let t=r.a.get(s||(s=r.ye));if(!t)return
...
console.log(e),o||u.showHUD(e),o=1};if(n.includes("%")&&/%[a-f0-9]{2}/i.test(n)&&(n=l.Xe(n)),
n&&!n.startsWith("^ "))p('"vimium://status" only accepts a list of hooked keys'),n="";else if(n){
let e=n.match(/<(?!<)(?:a-)?(?:c-)?(?:m-)?(?:s-)?(?:[a-z]\w+|[^\sA-Z])>|\S/g)
;n=e?e.join(" ").replace(/<(\S+)>/g,"$1"):""}let d=r.O.s,g=d.f,b=c.ss?1===g?g:(f=c.rs(d.Rl,d),
...
;for(let e of t.J){let s=e.s;!m&&c.ss&&(f=x.p=c.rs(s.Rl,s),$=null===f?0:f?1:2,1!==$&&s.f===$)||(s.f=$,e.postMessage(x))}
...
;let t=e.slice(0,l-1).split(e.lastIndexOf(" ",l-1)>=0?" ":"|").filter(e=>r.Il.map.has(e))
