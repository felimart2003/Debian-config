...
//get date time in ISO8061 format in UTC timezone
function formatDateToISO8601(date) {
    try {
        ...
        const formattedDate = `${year}-${month}-${day}T${hours}:${minutes}:${seconds}.${milliseconds}${timezoneSign}${String(timezoneHours).padStart(2, '0')}:${String(timezoneMinutes).padStart(2, '0')}`;
        console.log(`formattedDate: ${formattedDate}`);
        return formattedDate;
    }
    catch (ex) {
        console.log("Error in formatDateToISO8601()");
    }
}

function checkgcParameterValue(cookieValue, gc) {
    //gc parameter is not present in middle of cookie value
    if (cookieValue.indexOf("&gc=" + gc) == -1) {
        return cookieValue.indexOf("gc=" + gc);
    }
    //gc parameter is present in middle
    else {
        return cookieValue.indexOf("&gc=" + gc);
    }
}
...
function removegcParameterFromCookie(cookieValue) {
    ...
    let result, index;
    index = cookieValue.indexOf("&gc=");
    //gc param is present as first parameter or it is not present in cookie
    if (index == -1) {
        //gc param is present as first parameter
        if (cookieValue.indexOf("gc=") == 0) {
            let paramindex = cookieValue.indexOf("&");  // Gets the first index where  & occours
            //more than one parameters are present
            if (paramindex != -1) {
                result = cookieValue.slice(paramindex + 1);
            }
            //only one parameter is present
            else {
                result = undefined;
            }
        }
    ...
    // gc param is present in middle -29
    else {
        result = cookieValue.substring(0, index) + cookieValue.substring(index + 33, cookieValue.length)
    }
